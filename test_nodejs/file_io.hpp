	...
	*/
	...
	private:
		...
		void make_file_mode_(const char* mode) {
			if(std::strrchr(mode, 'b')) binary_mode_ = true;
			else binary_mode_ = false;
			if(std::strrchr(mode, 'w')) write_mode_ = true;
			else write_mode_ = false;
			if(std::strrchr(mode, 'r')) read_mode_ = true;
			else read_mode_ = false;
			if(std::strrchr(mode, 'a')) append_mode_ = true;
