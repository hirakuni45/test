    ...
    */
	...
	public:
		...
		bool insert(KEY key, PAD pad)
		{
			...
			uint32_t i = 0;
			if(size_ >= 2 && key >= cen_key_[0]) i = cen_org_[0];
			else if(size_ >= 4) {
				if(key >= cen_key_[1]) i = cen_org_[1];
				else if(key >= cen_key_[2]) i = cen_org_[2];
			}
			...
			if(size_ >= 2) {
				cen_key_[1] = core_[size_ / 2].key;
				cen_org_[1] = size_ / 2;
				if(size_ >= 4) {
					cen_key_[0] = core_[size_ / 4].key;
					cen_org_[0] = size_ / 4;
					cen_key_[2] = core_[size_ * 3 / 4].key;
					cen_org_[2] = size_ * 3 / 4;
				}
			}
