...
///	bool string_to_matrix4x4(const std::string& src, mtx::fmat4& dst);
	...
	*/
	...
	/*!
		...
		@param[in]	src	ソース文字列
		@param[in]	bks	バックスラッシュを無視する場合「true」
		@return ファイル名（ポインター）
	*/
	//-----------------------------------------------------------------//
	template <class T>
	...
	inline std::string get_file_name(const std::string& src, bool bks = false) {
		return get_file_nameT(src, bks);
	}
	inline wstring get_file_name(const wstring& src, bool bks = false) {
		return get_file_nameT(src, bks);
	}
	inline lstring get_file_name(const lstring& src, bool bks = false) {
		return get_file_nameT(src, bks);
	}
	...
	*/
	//-----------------------------------------------------------------//
	inline std::string get_file_base(const std::string& src, bool bks = false) {
